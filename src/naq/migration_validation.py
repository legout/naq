...
class MigrationValidationReport:
    ...
    def pre_integration_passed(self) -> bool:
        ...
        return all(r.status == ValidationStatus.PASS for r in self.pre_integration_results)
    
    @property
    def post_integration_passed(self) -> bool:
        """Check if all post-integration checks passed."""
        return all(r.status == ValidationStatus.PASS for r in self.post_integration_results)
...
class MigrationValidator:
    ...
    async def run_pre_integration_checks(self) -> List[ValidationResult]:
        ...
        for check in checks:
            try:
                ...
                logger.info(f"Pre-integration check '{result.name}': {result.status.value}")
    ...
    async def run_post_integration_checks(self) -> List[ValidationResult]:
        ...
        for check in checks:
            try:
                result = await check()
                results.append(result)
                logger.info(f"Post-integration check '{result.name}': {result.status.value}")
            except Exception as e:
    ...
    async def _verify_all_tests_pass(self) -> ValidationResult:
        ...
        try:
            ...
            import src.naq.services.base
            import src.naq.services.connection
            import src.naq.services.config
            import src.naq.services.jobs
            ...
            if hasattr(self.service_manager, 'cleanup'):
                try:
                    ...
                    if not callable(cleanup_method):
                        cleanup_issues.append("ServiceManager cleanup method is not callable")
                except Exception as e:
                    cleanup_issues.append(f"ServiceManager cleanup error: {str(e)}")
            
            # Check connection service cleanup
            connection_service = self.service_manager.get_service(ConnectionService)
            if connection_service and hasattr(connection_service, 'cleanup_idle_connections'):
                try:
                    # Test idle connection cleanup
                    if hasattr(connection_service, 'cleanup_idle_connections'):
                        cleanup_method = getattr(connection_service, 'cleanup_idle_connections')
                        if not callable(cleanup_method):
                            cleanup_issues.append("ConnectionService cleanup method is not callable")
                except Exception as e:
...
async def validate_migration(service_manager: ServiceManager) -> MigrationValidationReport:
