...
def scheduler(
    ...
    logger.info(f"Starting scheduler{f' instance {instance_id}' if instance_id else ''}")
    logger.info(f"NATS URL: {nats_url}")
    ...
    async def _list_scheduled_jobs():
        try:
            ...
            try:
                ...
                logger.error(f"Failed to access KV store '{SCHEDULED_JOBS_KV_NAME}': {e}")
            ...
            # Print header
            if detailed:
                print(f"{'JOB ID':<36} | {'QUEUE':<15} | {'STATUS':<10} | {'NEXT RUN':<25} | {'SCHEDULE TYPE':<15} | {'REPEATS LEFT':<12} | DETAILS")
                print("-" * 130)
            else:
                print(f"{'JOB ID':<36} | {'QUEUE':<15} | {'STATUS':<10} | {'NEXT RUN':<25} | {'SCHEDULE TYPE':<15}")
                print("-" * 100)
            ...
            for job in jobs_data:
                ...
                if next_run_ts:
                    next_run = datetime.datetime.fromtimestamp(next_run_ts, timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')
                else:
                    next_run = 'unknown'
                ...
                if detailed:
                    ...
                    if job.get('last_enqueued_utc'):
                        last_run = datetime.datetime.fromtimestamp(job.get('last_enqueued_utc'), timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')
                        details.append(f"last_run={last_run}")
                        
                    details_str = ', '.join(details)
                    print(f"{job_id:<36} | {queue_name:<15} | {status:<10} | {next_run:<25} | {schedule_type:<15} | {repeats:<12} | {details_str}")
                else:
                    print(f"{job_id:<36} | {queue_name:<15} | {status:<10} | {next_run:<25} | {schedule_type:<15}")
...
def job_control(
    ...
    if action not in ['cancel', 'pause', 'resume', 'reschedule']:
        logger.error(f"Invalid action '{action}'. Must be one of: cancel, pause, resume, reschedule")
        raise typer.Exit(code=1)
        
    # Validate parameters for reschedule
    if action == 'reschedule':
        if not any([cron, interval, repeat, next_run]):
            logger.error("Reschedule action requires at least one scheduling parameter: --cron, --interval, --repeat, or --next-run")
            raise typer.Exit(code=1)
        if cron and interval:
            logger.error("Cannot specify both --cron and --interval. Choose one scheduling method.")
            raise typer.Exit(code=1)
        ...
        try:
            ...
            if action == 'cancel':
                ...
                if result:
                    ...
                    print(f"Failed to pause job {job_id}. Job might not exist or was already paused.")
                ...
                if result:
                    ...
                    print(f"Failed to resume job {job_id}. Job might not exist or was not paused.")
                ...
                if next_run:
                    try:
                        # Parse ISO format
                        next_run_dt = datetime.datetime.fromisoformat(next_run.replace('Z', '+00:00'))
                        # Convert to UTC timestamp
                        updates['scheduled_timestamp_utc'] = next_run_dt.timestamp()
                    except ValueError as e:
                        logger.error(f"Invalid next_run format: {e}. Use ISO format (e.g., '2023-01-01T12:00:00Z')")
                        raise typer.Exit(code=1)
