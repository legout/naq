...
"""
Synchronous wrapper functions for NAQ queue operations.

This module provides synchronous wrapper functions that internally use
thread-local connections for efficient reuse across multiple calls from
the same thread.
"""
...
from ..models.enums import RetryDelayType
from ..settings import DEFAULT_NATS_URL, DEFAULT_QUEUE_NAME
from ..utils.async_helpers import async_to_sync as run_async_from_sync
...
def purge_queue_sync(
    ...
    async def _main():
        count = await purge_queue(
            queue_name=queue_name, config=config, nats_url=nats_url, prefer_thread_local=True
        )
        return count
...
def cancel_scheduled_job_sync(
    ...
    async def _main():
        res = await cancel_scheduled_job(
            job_id, config=config, nats_url=nats_url, prefer_thread_local=True
        )
        return res
    ...
    **updates: Any
    ...
    async def _main():
        res = await modify_scheduled_job(
            job_id, config=config, nats_url=nats_url, prefer_thread_local=True, **updates
        )
        return res
