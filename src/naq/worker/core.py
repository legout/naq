...
class Worker:
    ...
    def __init__(
        ...
        self._logger.info("Worker initialized", worker_id=self.worker_id, queues=self.queue_names)
    ...
    def _resolve_ack_wait_seconds(self, queue_name: QueueName) -> int:
        ...
        try:
            ...
            if queue_name in ACK_WAIT_PER_QUEUE:
                v = ensure_type(ACK_WAIT_PER_QUEUE[queue_name], int, "env_ack_wait_value")
                return v if v > 0 else DEFAULT_ACK_WAIT_SECONDS
        except Exception as e:
    ...
    async def _subscribe_to_queue(self, queue_name: QueueName) -> None:
        ...
        """
        with self._logger.operation_context("subscribe_to_queue", queue_name=queue_name):
            try:
                if not self._js:
                ...
                while self._running:
                    ...
                    try:
                        ...
                        wrapped_error = wrap_naq_exception(e, "Error fetching from consumer")
    ...
    async def run(self) -> None:
        ...
        with self._logger.operation_context("worker_run", worker_id=self.worker_id):
            try:
                ...
                self._logger.info("Shutdown signal received, waiting for tasks to complete")
                ...
                if active_tasks > 0:
                    self._logger.info("Waiting for active jobs to finish", active_tasks=active_tasks)
                    ...
                    try:
                        ...
                        self._logger.warning("Timeout waiting for active jobs to finish")
                ...
                wrapped_error = wrap_naq_exception(e, "Worker run loop encountered an error")
                self._error_handler.handle_error(
                    wrapped_error,
                    {"worker_id": self.worker_id}
                )
    ...
    async def _close(self) -> None:
        ...
        with self._logger.operation_context("worker_close"):
            ...
            for queue_name, consumer in self._consumers.items():
                try:
                    self._logger.debug("Unsubscribing consumer for queue", queue_name=queue_name)
                    await consumer.unsubscribe()
                    self._logger.debug("Draining consumer for queue", queue_name=queue_name)
    ...
    def signal_handler(self, sig, frame) -> None:
        ...
        self._logger.warning("Received termination signal, initiating graceful shutdown", signal=sig)
        self._running = False
        self._shutdown_event.set()
    ...
    def install_signal_handlers(self) -> None:
        ...
        try:
            ...
            if threading.current_thread() is not threading.main_thread():
                self._logger.warning("Skipping installation of signal handlers because we are not in the main thread")
                return
            ...
            wrapped_error = wrap_naq_exception(e, "Unexpected error installing signal handlers")
            self._error_handler.handle_error(wrapped_error)
    ...
    async def list_workers(nats_url: str = DEFAULT_NATS_URL) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                wrapped_error = wrap_naq_exception(e, "Error cleaning up service manager")
