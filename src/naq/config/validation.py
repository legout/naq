...
def validate_positive_integer(
    value: int, field_name: str, min_value: int = 1, max_value: Optional[int] = None
) -> None:
    """
...
def validate_non_negative_integer(
    value: int, field_name: str, max_value: Optional[int] = None
) -> None:
    """
...
def validate_positive_float(
    ...
    if max_value is not None and value_float > max_value:
        raise ValidationError(
            f"{field_name} must be at most {max_value}, got {value_float}"
        )
...
def validate_list_of_strings(
    value: List[Any], field_name: str, allow_empty: bool = True
) -> None:
    """
...
def validate_dict_string_keys(
    value: Dict[Any, Any], field_name: str, allow_empty: bool = True
) -> None:
    """
...
def validate_connection_config(config: Dict[str, Any]) -> None:
    ...
    if "servers" in config:
        validate_list_of_strings(
            config["servers"], "connection.servers", allow_empty=False
        )
    ...
    if "max_reconnect_attempts" in config:
        validate_non_negative_integer(
            config["max_reconnect_attempts"], "connection.max_reconnect_attempts"
        )

    if "reconnect_time_wait" in config:
        validate_positive_float(
            config["reconnect_time_wait"], "connection.reconnect_time_wait"
        )

    if "connection_timeout" in config:
        validate_positive_float(
            config["connection_timeout"], "connection.connection_timeout"
        )

    if "drain_timeout" in config:
        validate_positive_float(config["drain_timeout"], "connection.drain_timeout")
...
def validate_job_service_config(config: Dict[str, Any]) -> None:
    ...
    if "queue_name" in config:
        validate_string_length(
            config["queue_name"], "job_service.queue_name", min_length=1, max_length=100
        )

    # Validate default_job_ttl
    if "default_job_ttl" in config:
        validate_non_negative_integer(
            config["default_job_ttl"], "job_service.default_job_ttl"
        )

    # Validate max_retries
    if "max_retries" in config:
        validate_non_negative_integer(config["max_retries"], "job_service.max_retries")

    # Validate result_expiry
    if "result_expiry" in config:
        validate_non_negative_integer(
            config["result_expiry"], "job_service.result_expiry"
        )

    # Validate boolean fields
    if "enable_result_backend" in config:
        validate_boolean(
            config["enable_result_backend"], "job_service.enable_result_backend"
        )

    if "enable_dead_letter_queue" in config:
        validate_boolean(
            config["enable_dead_letter_queue"], "job_service.enable_dead_letter_queue"
        )
...
def validate_worker_service_config(config: Dict[str, Any]) -> None:
    ...
    if "default_worker_ttl" in config:
        validate_positive_integer(
            config["default_worker_ttl"], "worker_service.default_worker_ttl"
        )

    if "heartbeat_interval" in config:
        validate_positive_float(
            config["heartbeat_interval"], "worker_service.heartbeat_interval"
        )

    # Validate boolean fields
    if "enable_worker_registration" in config:
        validate_boolean(
            config["enable_worker_registration"],
            "worker_service.enable_worker_registration",
        )

    if "enable_event_logging" in config:
        validate_boolean(
            config["enable_event_logging"], "worker_service.enable_event_logging"
        )

    if "auto_create_buckets" in config:
        validate_boolean(
            config["auto_create_buckets"], "worker_service.auto_create_buckets"
        )
...
def validate_scheduler_service_config(config: Dict[str, Any]) -> None:
    ...
    if "lock_ttl" in config:
        validate_positive_float(config["lock_ttl"], "scheduler_service.lock_ttl")

    if "lock_renewal_interval" in config:
        validate_positive_float(
            config["lock_renewal_interval"], "scheduler_service.lock_renewal_interval"
        )

    if "max_schedule_failures" in config:
        validate_non_negative_integer(
            config["max_schedule_failures"], "scheduler_service.max_schedule_failures"
        )

    # Validate boolean fields
    if "enable_leader_election" in config:
        validate_boolean(
            config["enable_leader_election"], "scheduler_service.enable_leader_election"
        )

    if "auto_create_buckets" in config:
        validate_boolean(
            config["auto_create_buckets"], "scheduler_service.auto_create_buckets"
        )
...
def validate_stream_service_config(config: Dict[str, Any]) -> None:
    ...
    if "max_msgs" in config and config["max_msgs"] is not None:
        validate_positive_integer(config["max_msgs"], "stream_service.max_msgs")

    if "max_bytes" in config and config["max_bytes"] is not None:
        validate_positive_integer(config["max_bytes"], "stream_service.max_bytes")

    if "max_age" in config and config["max_age"] is not None:
        validate_positive_float(config["max_age"], "stream_service.max_age")
    ...
    if "auto_create_stream" in config:
        validate_boolean(
            config["auto_create_stream"], "stream_service.auto_create_stream"
        )
...
def validate_kv_store_config(config: Dict[str, Any]) -> None:
    ...
    if "bucket_name" in config:
        validate_string_length(
            config["bucket_name"], "kv_store.bucket_name", min_length=1, max_length=100
        )

    # Validate numeric fields
    if "ttl" in config and config["ttl"] is not None:
        validate_non_negative_integer(config["ttl"], "kv_store.ttl")

    if "history" in config and config["history"] is not None:
        validate_positive_integer(config["history"], "kv_store.history")

    if "replicas" in config and config["replicas"] is not None:
        validate_positive_integer(config["replicas"], "kv_store.replicas")
...
def validate_event_service_config(config: Dict[str, Any]) -> None:
    ...
    if "max_events" in config:
        validate_positive_integer(config["max_events"], "event_service.max_events")

    if "event_ttl" in config:
        validate_non_negative_integer(config["event_ttl"], "event_service.event_ttl")

    # Validate boolean fields
    if "enable_event_logging" in config:
        validate_boolean(
            config["enable_event_logging"], "event_service.enable_event_logging"
        )

    if "auto_create_bucket" in config:
        validate_boolean(
            config["auto_create_bucket"], "event_service.auto_create_bucket"
        )
...
def validate_naq_config(config: Dict[str, Any]) -> None:
    ...
    if "nats" in config:
        ...
        if "servers" in config["nats"]:
            validate_list_of_strings(
                config["nats"]["servers"], "nats.servers", allow_empty=False
            )
