...
class SerializationHelper:
    ...
    def safe_serialize(
        ...
        if serializer not in ("pickle", "json"):
            raise ValueError(
                f"Unsupported serializer: {serializer}. Use 'pickle' or 'json'."
            )
        ...
        try:
            ...
            if fallback_serializer and fallback_serializer != serializer:
                ...
                raise SerializationError(
                    f"Failed to serialize data with {serializer}: {str(e)}"
                ) from e
        ...
        try:
            if serializer == "pickle":
                ...
                if not isinstance(bytes_data, (str, bytes)):
                    raise TypeError(
                        "JSON deserialization requires string or bytes data"
                    )
            ...
            raise SerializationError(
                f"Failed to deserialize data with {serializer}: {str(e)}"
            ) from e
