...
This module contains all CLI commands related to job and queue management.
"""
...
def job_control(
    ...
    next_run: Optional[str] = typer.Option(
        ...
        "--next-run",
        help=(
            "Next run time (ISO format, e.g. '2023-01-01T12:00:00Z') for "
            "reschedule action"
        ),
    ),
    log_level: Optional[str] = typer.Option(
    ...
    if action == "reschedule":
        ...
        if cron and interval:
            logger.error(
                "Cannot specify both --cron and --interval. Choose one "
                "scheduling method."
            )
            raise typer.Exit(code=1)
    ...
    try:
        if action == "cancel":
            ...
            if result:
                ...
                console.print(
                    f"[yellow]Failed to pause job {job_id}. Job might not exist or "
                    f"was already paused.[/yellow]"
                )
            ...
            if result:
                ...
                console.print(
                    f"[yellow]Failed to resume job {job_id}. Job might not exist or "
                    f"was not paused.[/yellow]"
                )
            ...
            if next_run:
                try:
                    ...
                    logger.error(
                        f"Invalid next_run format: {e}. Use ISO format (e.g., "
            ...
            if result:
                ...
                console.print(
                    f"[yellow]Failed to reschedule job {job_id}. Job might not "
                    f"exist.[/yellow]"
                )
    except Exception as e:
