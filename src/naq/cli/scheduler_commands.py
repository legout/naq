...
This module contains all CLI commands related to scheduler management.
"""
...
def scheduler(
    ...
    instance_id: Optional[str] = typer.Option(
        ...
        "-i",
        help=(
            "Optional unique ID for this scheduler instance (for high availability)."
        ),
    ),
    ...
    async def _list_scheduled_jobs_async():
        try:
            ...
            try:
                ...
                console.print(
                    "[yellow]No scheduled jobs found or cannot access job "
                    "store.[/yellow]"
                )
                return
