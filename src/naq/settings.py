...
class NATSConnectionConfig(msgspec.Struct):
    ...
    max_reconnect_attempts: int = msgspec.field(
        default_factory=lambda: int(os.getenv("NAQ_MAX_RECONNECT_ATTEMPTS", "5"))
    )
    reconnect_time_wait: float = msgspec.field(
        default_factory=lambda: float(os.getenv("NAQ_RECONNECT_TIME_WAIT", "2.0"))
    )
    connection_timeout: float = msgspec.field(
        default_factory=lambda: float(os.getenv("NAQ_CONNECTION_TIMEOUT", "10.0"))
    )
    ping_interval: float = msgspec.field(
        default_factory=lambda: float(os.getenv("NAQ_PING_INTERVAL", "30.0"))
    )
    max_outstanding_pings: int = msgspec.field(
        default_factory=lambda: int(os.getenv("NAQ_MAX_OUTSTANDING_PINGS", "3"))
    )
    prefer_thread_local: bool = msgspec.field(
        default_factory=lambda: os.getenv("NAQ_PREFER_THREAD_LOCAL", "false").lower()
        == "true"
    )
    name: str = msgspec.field(
        default_factory=lambda: os.getenv("NAQ_CLIENT_NAME", "naq_client")
    )
    no_randomize: bool = msgspec.field(
        default_factory=lambda: os.getenv("NAQ_NO_RANDOMIZE", "false").lower() == "true"
    )
    tls: Optional[dict[str, Any]] = None
    user: Optional[str] = msgspec.field(default_factory=lambda: os.getenv("NAQ_USER"))
    password: Optional[str] = msgspec.field(
        default_factory=lambda: os.getenv("NAQ_PASSWORD")
    )
    ...
    credentials: Optional[str] = msgspec.field(
        default_factory=lambda: os.getenv("NAQ_CREDENTIALS")
    )
    ...
    def from_env(cls) -> "Config":
        ...
        return cls(
            ...
            scheduler_lock_ttl_seconds=int(
                os.getenv("NAQ_SCHEDULER_LOCK_TTL", SCHEDULER_LOCK_TTL_SECONDS)
            ),
            scheduler_lock_renew_interval_seconds=int(
                os.getenv(
            ...
            max_schedule_failures=int(
                os.getenv("NAQ_MAX_SCHEDULE_FAILURES", MAX_SCHEDULE_FAILURES)
            )
            if os.getenv("NAQ_MAX_SCHEDULE_FAILURES")
            else None,
            job_status_ttl_seconds=int(
                os.getenv("NAQ_JOB_STATUS_TTL", JOB_STATUS_TTL_SECONDS)
            ),
            default_result_ttl_seconds=int(
                os.getenv("NAQ_DEFAULT_RESULT_TTL", DEFAULT_RESULT_TTL_SECONDS)
            ),
            worker_ttl_seconds=int(
                os.getenv("NAQ_WORKER_TTL", DEFAULT_WORKER_TTL_SECONDS)
            ),
            ...
            default_ack_wait_seconds=int(
                os.getenv("NAQ_DEFAULT_ACK_WAIT", DEFAULT_ACK_WAIT_SECONDS)
            ),
            ...
            log_to_file_enabled=os.getenv("NAQ_LOG_TO_FILE_ENABLED", "false").lower()
            == "true",
