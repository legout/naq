...
)  # SCHEDULED_JOB_STATUS_FAILED, # Import statuses
from .settings import (
    DEFAULT_QUEUE_NAME,
    JOB_SERIALIZER,
    NAQ_PREFIX,
...
class Queue:
    ...
    async def _get_scheduled_kv(self) -> KeyValue:
        ...
        try:
            ...
            try:
                logger.info(
                    f"KV store '{SCHEDULED_JOBS_KV_NAME}' not found, attempting creation..."
                )
    ...
    async def schedule(
        ...
        *args: Any,
        ...
        repeat: Optional[
            int
        ] = None,  # None for infinite, 0/1 for once (use enqueue_*), >1 for limited repeats
        max_retries: Optional[int] = 0,
        retry_delay: RetryDelayType = 0,
        **kwargs: Any,
    ) -> Job:
        ...
        print(
            f"Scheduled recurring job {job.job_id} ({func.__name__}) starting at {datetime.datetime.fromtimestamp(first_run_ts, timezone.utc)} on queue '{self.name}'"
        )
        return job
    ...
    async def _update_scheduled_job_status(self, job_id: str, status: str) -> bool:
        ...
        try:
            ...
            if "wrong last sequence" in str(e).lower():
                print(
                    f"Warning: Concurrent modification detected for job '{job_id}'. Update failed. Please retry."
                )
    ...
    async def pause_scheduled_job(self, job_id: str) -> bool:
        ...
        return await self._update_scheduled_job_status(
        ...
        try:
            ...
            if needs_next_run_recalc:
                ...
                if next_run_ts is not None:
                    ...
                    print(
                        f"Warning: Could not determine next run time for job '{job_id}' after modification. Check parameters."
                    )
