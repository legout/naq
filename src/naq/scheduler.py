...
from .settings import DEFAULT_NATS_URL  # SCHEDULED_JOB_STATUS_ACTIVE,
...
class Scheduler:
    ...
    async def _connect(self):
        ...
        if self._nc is None or not self._nc.is_connected:
            ...
            if self._enable_ha:
                try:
                    self._lock_kv = await self._js.key_value(
        ...
        try:
            ...
            try:
                ...
                if entry:
                    ...
                    if (
                        time.time() - lock_time < self._lock_ttl
                        and lock_owner != self._instance_id
                    ):
                        logger.debug(
            ...
            logger.info(
                f"Acquired scheduler leader lock. This instance ({self._instance_id}) is now the leader."
            )
            return True
        ...
        while self._running and self._is_leader:
            try:
                if self._lock_kv:
                    ...
                    await self._lock_kv.put(SCHEDULER_LOCK_KEY, serialized_lock)
                    logger.debug(
                        f"Renewed leader lock. Next renewal in {self._lock_renew_interval}s"
                    )

                # Wait for renewal interval or until shutdown
                try:
                    await asyncio.wait_for(
                        self._shutdown_event.wait(), timeout=self._lock_renew_interval
                    )
    ...
    async def _check_and_process_jobs(self):
        ...
        if not self._kv or not self._js:
            logger.warning(
                "KV store or JetStream context not available, skipping check."
            )
            return

        # Only log the start of the check if we are leader
        # logger.debug("Scheduler checking for jobs...") # Moved logging inside leader check
        ...
        try:
            # This is the most likely source of the "no keys found" error
            keys = await self._kv.keys()

            if not keys:
                logger.debug("No scheduled jobs found in KV store.")
                return  # Exit check cycle if no keys

            logger.debug(f"Found {len(keys)} potential scheduled jobs.")

            for key_bytes in keys:
                # Only process if we're still the leader (check moved from run loop for safety)
                ...
                try:
                    ...
                    if entry is None:
                        continue  # Should not happen if key exists, but check
                    ...
                    if enqueue_success:
                        ...
                        failure_count = (
                            schedule_data.get("schedule_failure_count", 0) + 1
                        )
                        ...
                        if (
                            MAX_SCHEDULE_FAILURES
                            and failure_count >= MAX_SCHEDULE_FAILURES
                        ):
                            logger.warning(
                    ...
                    if cron:
                        if croniter is None:
                            ...
                            base_dt = datetime.datetime.fromtimestamp(
                                scheduled_ts, timezone.utc
                            )
                            cron_iter = croniter(cron, base_dt)
                            next_scheduled_ts = cron_iter.get_next(
                                datetime.datetime
                            ).timestamp()
                    ...
                    if next_scheduled_ts is not None:
                        ...
                        if repeat is not None:
                            if repeat > 1:
                                ...
                                schedule_data["scheduled_timestamp_utc"] = (
                                    next_scheduled_ts
                                )
            ...
            # --- Explicitly check for and ignore the 'no keys found' error ---
            # Convert error to string and check for the specific message
            error_message = str(e).lower()
            if "no keys found" in error_message:
                # This is the expected case when the bucket is empty, log as debug
                logger.debug(
                    "No scheduled jobs found (received NATS 'no keys found' message)."
                )
            else:
                # Log other NATS errors as actual errors
        ...
        if processed_count > 0 or error_count > 0:
            logger.info(
                f"Scheduler processed {processed_count} ready jobs, encountered {error_count} errors."
            )
