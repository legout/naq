...
class LeaderElection:
    ...
    async def try_become_leader(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Acquired scheduler leader lock. This instance ({self.instance_id}) is now the leader.")
            return True
    ...
    async def start_renewal_task(self, running_flag: bool) -> None:
        ...
        self._lock_renewal_task = asyncio.create_task(self._renew_leader_lock(running_flag))
        ...
        while running_flag and self._is_leader:
            try:
                if self._lock_kv:
                    ...
                    serialized_lock = cloudpickle.dumps(lock_data)
                    await self._lock_kv.put(SCHEDULER_LOCK_KEY, serialized_lock)
                    logger.debug(f"Renewed leader lock. Next renewal in {self.lock_renew_interval}s")
...
class ScheduledJobProcessor:
    ...
    async def process_jobs(self, is_leader: bool) -> tuple[int, int]:
        ...
        try:
            ...
            for key_bytes in keys:
                ...
                if not is_leader:
                    logger.info("Lost leadership during job processing loop, stopping check.")
                    break
            ...
            if "no keys found" in error_message:
                logger.debug("No scheduled jobs found (received NATS 'no keys found' message).")
            else:
    ...
    async def _process_single_job(self, key_bytes: bytes, now_ts: float) -> tuple[int, int]:
        """
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Failed to delete corrupted schedule entry '{key}': {del_e}")
            return 0, 1
        except Exception as e:
        ...
        try:
            ...
            while self._running:
                ...
                if self._enable_ha:
                    if not self.is_leader:
                        ...
                        if await self._leader_election.try_become_leader():
                            ...
                            await self._leader_election.start_renewal_task(self._running)
                else:
                ...
                if self.is_leader and self._job_processor:
                    processed, errors = await self._job_processor.process_jobs(self.is_leader)
                    # Log summary only if something happened
                    if processed > 0 or errors > 0:
                        logger.info(f"Scheduler processed {processed} ready jobs, encountered {errors} errors.")
                else:
