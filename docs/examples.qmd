...
The `ConnectionService` manages the NATS connection and provides a centralized way to connect to the NATS server.
...
### Using KVStoreService

The `KVStoreService` provides a key-value store interface using NATS KV.
...
async def main():
    # Create KV store configuration
    kv_config = KVStoreServiceConfig(
        bucket_name="user_data",
        history=10,  # Keep 10 revisions of each key
...
The `EventService` provides a structured way to log and retrieve events.
...
async def main():
    ...
    kv_config = KVStoreServiceConfig(bucket_name="events")
    kv_store_service = KVStoreService(
        config=ServiceConfig(custom_settings=kv_config.as_dict())
    )
    await kv_store_service.initialize()
    
    # Create event service configuration
    event_config = EventServiceConfig(
        event_bucket_name="system_events",
...
The `StreamService` provides a way to work with NATS JetStream streams.
...
async def main():
    ...
    # Create stream service configuration
    stream_config = StreamServiceConfig(
        stream_name="orders",
        subjects=["orders.>"],  # Match all subjects starting with "orders."
        retention_limit=1000,   # Keep up to 1000 messages
...
The `WorkerService` provides a way to manage workers for processing jobs.
...
async def main():
    ...
    kv_store_config = KVStoreServiceConfig(bucket_name="worker_data")
    kv_store_service = KVStoreService(
...
The `SchedulerService` provides a way to manage scheduled jobs.
...
async def main():
    ...
    kv_store_config = KVStoreServiceConfig(bucket_name="scheduler_data")
    kv_store_service = KVStoreService(
        config=ServiceConfig(custom_settings=kv_store_config.as_dict())
    )
    await kv_store_service.initialize()
    ...
    # Create scheduler service configuration
    scheduler_config = SchedulerServiceConfig(
        scheduler_name="main_scheduler",
        check_interval=5,  # Check for scheduled jobs every 5 seconds
...
The `ServiceManager` provides a centralized way to manage all services.
...
from naq.services.events import EventServiceConfig
from naq.services.streams import StreamServiceConfig
...
async def main():
    ...
    services_config = {
        ...
        "kv_store": KVStoreServiceConfig(
            bucket_name="app_data"
        ),
        "events": EventServiceConfig(
            event_bucket_name="app_events"
        ),
        "streams": StreamServiceConfig(
            stream_name="app_streams",
            subjects=["app.>"]
        ),
        "worker": WorkerServiceConfig(
            worker_name="app_worker",
            queues=["default"]
        ),
        "scheduler": SchedulerServiceConfig(
            scheduler_name="app_scheduler",
            check_interval=10
        )
    }
    ...
    service_manager = ServiceManager(
        config=ServiceConfig(custom_settings=services_config)
    )
...
### Composing Services for a Complete Workflow

Here's an example of how to compose multiple services to create a complete workflow:
